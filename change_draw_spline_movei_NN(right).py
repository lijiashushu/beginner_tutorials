...
class moveit_trajectory(object):
    def __init__(self, Armname):
        ...
        pose_init = self.limb.endpoint_pose()
        print pose_init
        print pose_init['orientation'][0]
        pose_target = geometry_msgs.msg.Pose()
        pose_target.orientation.x = pose_init['orientation'][0]
        pose_target.orientation.y = pose_init['orientation'][1]
        pose_target.orientation.z = pose_init['orientation'][2]
        pose_target.orientation.w = pose_init['orientation'][3]
        pose_target.position.x = pose_init['position'][0]+0.3
        pose_target.position.y = pose_init['position'][1]
        pose_target.position.z = pose_init['position'][2]
...
def main():
    ...
    while point_now < point_sum:
        if not rospy.is_shutdown():
            ...
            '''计算alpha'''
            for aaa in range(0, 7):
                alpha[aaa] = -udp.controller.torController[aaa].get_kp() * z1[aaa] + insert.vel_result[point_now-1][aaa]
            ...
            if draw_picture == True:
                if a == 0:
                    ...
                    for key in get_pose:
                        ...
                        joint_req_pose_display[temp, a] = insert.pos_result[point_now-1][temp]
